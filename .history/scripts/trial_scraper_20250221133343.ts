...
async function parseScorecard(matchId: string): Promise<{
  ...
  try {
    ...
    for (const inningsElem of $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').toArray()) {
      ...
      // Collect all players from match data
      battingData.forEach(player => addNameVariations(player.playerName));
      bowlingData.forEach(player => addNameVariations(player.playerName));
      dnbData.forEach(player => addNameVariations(player.playerName));
