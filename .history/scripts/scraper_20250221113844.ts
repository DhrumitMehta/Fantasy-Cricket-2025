    ...
    $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').each((_, inningsElem) => {
      ...
      if (inningsText.includes('Batter') || inningsText.includes('Batsman')) {
        ...
        $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
          ...
          if (cols.length >= 7) {
            const playerName = cols.eq(0).text().trim();
            const dismissalInfo = cols.eq(1).text().trim();
            ...
            if (playerName && !playerName.includes('Did not Bat')) {
              const fullName = cleanPlayerName(playerName);
              const playerLink = cols.eq(0).find('a').attr('href');
              const playerId = playerLink ? playerLink.split('/')[2] : 'unknown';
              nameMapping[playerName.toLowerCase()] = fullName;
              ...
              // Update batting specific stats
              playerData.runs = runs;
              playerData.balls = balls;
              playerData.fours = fours;
              playerData.sixes = sixes;
              playerData.strikeRate = strikeRate;

              battingData.push(playerData);
            }
          }
        });
      }
      ...
      if ($(inningsElem).find('.cb-scrd-sub-hdr').text().includes('Bowler')) {
        $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
          const cols = $(row).find('.cb-col');
          if (cols.length >= 8) {  // Make sure it's a bowling row
            const playerName = cols.eq(0).text().trim();
            ...
            if (playerName) {
              const fullName = nameMapping[playerName.toLowerCase()] || playerName;
              const playerLink = cols.eq(0).find('a').attr('href');
              const playerId = playerLink ? playerLink.split('/')[2] : 'unknown';
              
              // Create player data with bowling stats
              const playerData: PlayerData = {
                fullName,
                shortName: playerName,
                playerId,
                team: teamNames[currentInnings % 2],  // Bowling team is opposite of batting team
                matchId,
                innings: currentInnings,
                // Initialize batting stats to 0
                runs: 0,
                balls: 0,
                fours: 0,
                sixes: 0,
                strikeRate: 0,
                // Initialize bowling stats to 0
                overs: Number(overs),
              ...
              getBowlerDotBalls(matchId, playerId, currentInnings.toString())
                .then(dotBalls => {
                  playerData.dotBalls = dotBalls;
                })
                .catch(error => {
                  console.error(`Error getting dot balls for ${fullName}:`, error);
                  playerData.dotBalls = 0;
                });

              bowlingData.push(playerData);
            }
          }
        });
      }
      ...
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        ...
        if (playerLink) {
          ...
          const playerData: PlayerData = {
            ...
            shortName,
            playerId,
            team: teamNames[currentInnings - 1],
            matchId,
            innings: currentInnings,
            // Initialize batting stats to 0
            runs: 0,
          ...
          // Process catches
          const catchMatch = dismissalText.match(/c (.*?)\s+(?=b\s+)/);
          if (catchMatch) {
            const fielderName = catchMatch[1].trim();
            playerData.catches++;
            console.log(`Found catch: ${playerName} caught by ${fielderName}`);
          }
          
          // Process caught & bowled
          if (dismissalText.includes('c & b')) {
            const fielderName = dismissalText.split('c & b')[1].trim();
            playerData.catches++;
            console.log(`Found caught & bowled: ${playerName} caught by ${fielderName}`);
          }
          
          // Process stumpings
          const stumpMatch = dismissalText.match(/st (\w+ \w+)/);
          if (stumpMatch) {
            playerData.stumpings++;
            console.log(`Found stumping: ${playerName}`);
          }
          
          // Process run outs
          const runOutMatch = dismissalText.match(/run out \(([\w\s/]+)\)/);
          if (runOutMatch) {
            const fielders = runOutMatch[1].split('/').map(f => f.trim());
            playerData.runOuts += fielders.length;
            console.log(`Found run out: ${playerName} (${fielders.join('/')})`);
          }
          
          fieldingData.push(playerData);
        }
      });
    });

    // Get Player of the Match
    const potmUrl = `https://www.cricbuzz.com/cricket-scores/${matchId}`;
    const potmResponse = await fetchWithRetry(potmUrl);
    const potm$ = cheerio.load(potmResponse);
    const potmDiv = potm$('.cb-col.cb-col-50.cb-mom-itm');
    if (potmDiv.length) {
      const potmName = potmDiv.find('a.cb-link-undrln').text().trim();
      const potmLink = potmDiv.find('a.cb-link-undrln').attr('href');
      
      if (potmLink) {
        const potmId = potmLink.split('/')[2];
        const potmFullName = cleanPlayerName(potmName);
        
        const potmPlayerData: PlayerData = {
          fullName: potmFullName,
          shortName: potmName,
          playerId: potmId,
          team: '',  // We might want to look this up from the match data
          matchId,
          innings: 0,
          // Initialize batting stats to 0
          runs: 0,
        ...
        fieldingData.push(potmPlayerData);
        console.log(`Player of the Match: ${potmName}`);
      }
    }

    // Fetch dot balls for bowlers
    const bowlerPromises = bowlingData.map(async (player) => {
      const dotBalls = await getBowlerDotBalls(matchId, player.playerId || '', player.innings.toString());
      player.dotBalls = dotBalls;
      console.log(`Fetched dot balls for ${player.fullName}: ${dotBalls}`);
    });
    await Promise.all(bowlerPromises);
...
function updateFieldingStats(dismissalInfo: string, fieldingData: PlayerData[], matchId: string, team: string, nameMapping: NameMapping): void {
  ...
  if (dismissalInfo.includes('c & b')) {
    ...
    addFieldingContribution(fieldingData, mappedName, 'catch', matchId, team, 0);
  } else {
  ...
  if (stumpMatch) {
    ...
    const mappedName = nameMapping[fielder.toLowerCase()] || properCase(fielder);
    addFieldingContribution(fieldingData, mappedName, 'stumping', matchId, team, 0);
  }
  ...
  if (runOutMatch) {
    ...
    fielders.forEach(fielder => {
      const mappedName = nameMapping[fielder.toLowerCase()] || properCase(fielder);
      addFieldingContribution(fieldingData, mappedName, 'runOut', matchId, team, 0);
    });
  }
}
...
function addFieldingContribution(
  ...
  // First try to find the player in the existing data
  let player: PlayerData | undefined = fieldingData.find(p => 
    p.fullName.toLowerCase() === playerName.toLowerCase() || 
    p.shortName.toLowerCase() === playerName.toLowerCase()
  );
  
  if (!player) {
    // Only create a new player if they don't exist
    player = {
      fullName: playerName,
      shortName: playerName,
      playerId: 'unknown',
  ...
  // else: player exists, just update their fielding stats

  // Update only the relevant fielding stat
  switch (type) {
    case 'catch':
      player.catches++;
      break;
...
function calculatePoints(playerData: PlayerData): {
  ...
  if (playerData.runs !== undefined) {
    ...
    console.log(`Base points for ${playerData.runs} runs: ${playerData.runs}`);
    ...
    const milestoneBonus = Math.floor(playerData.runs / 25) * 10;
    points.battingPoints += milestoneBonus;
    console.log(`Milestone bonus (${Math.floor(playerData.runs / 25)} × 10): ${milestoneBonus}`);
  ...
  if (playerData.wickets !== undefined) {
    ...
    if (playerData.overs && playerData.overs >= 1) {
      const economy = (playerData.runs || 0) / playerData.overs;
  ...
  points.fieldingPoints = (playerData.catches || 0) * 10 +
                         (playerData.stumpings || 0) * 12 +
                         (playerData.runOuts || 0) * 12;
...
function mergePlayerStats(battingData: PlayerData[], bowlingData: PlayerData[], fieldingData: PlayerData[], dnbData: PlayerData[]): PlayerData[] {
  ...
  const mergePlayer = (player: PlayerData) => {
    ...
    if (player.runs !== undefined) {
      mergedPlayers[key].runs = player.runs;
    ...
    if (player.overs !== undefined) {
      ...
      mergedPlayers[key].runsConceded = player.runs || 0;
  ...
  try {
    ...
    for (const match of matchesToProcess) {
      ...
      try {
        ...
        const html = await fetchWithRetry(match.scorecard_url);
        const {
          battingData,
          bowlingData,
          fieldingData,
