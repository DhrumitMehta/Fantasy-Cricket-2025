...
async function parseScorecard(html: string, matchId: string): Promise<{
  ...
  $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').each((_, inningsElem) => {
    ...
    if ($(inningsElem).find('.cb-scrd-sub-hdr').text().includes('Bowler')) {
      ...
      // Use Promise.all to fetch dot balls for all bowlers in parallel
      const bowlerPromises = [];
      ...
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        ...
        // Extract player ID from the highlights link
        const highlightsLink = cols.last().find('a').attr('href');
        const playerId = highlightsLink ? highlightsLink.split('/')[5] : null;
