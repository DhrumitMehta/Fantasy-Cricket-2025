...
interface PlayerData {
  ...
  wides?: number;
  economy?: number;
  dotBalls?: number; // Add dotBalls field
  // Fielding stats
  catches?: number;
  stumpings?: number;
  runOuts?: number;
...
async function fetchWithRetry(url: string, maxRetries = 5, initialDelay = 1000): Promise<string> {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      // Remove timeout since it's not a valid RequestInit property
      ...
      return text;
    } catch (error: any) { // Type error as any to access .code and .message
      lastError = error;
      ...
      if (error.code === 'ENOTFOUND') {
        ...
        } catch (altError: any) { // Type altError as any
