...
async function getBowlerDotBalls(matchId: string, playerId: string, inningsNo: string): Promise<number> {
  try {
    ...
    // Find the dot balls count
    const dotBalls = $('.cb-col.cb-col-40.text-bold').filter((_, elem) => {
      return $(elem).text().trim() === 'Dot Balls';
    }).next().text();
    
    const dotBallCount = parseInt(dotBalls) || 0;
...
async function parseScorecard(html: string, matchId: string): Promise<{
  ...
  // Log all bowling data for verification
  console.log('\nAll bowling data collected:');
  bowlingData.forEach(bowler => {
    console.log(`${bowler.fullName} (${bowler.team}):`, {
      overs: bowler.overs,
