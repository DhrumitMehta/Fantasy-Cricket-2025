    ...
    $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
      ...
      if (dismissalText.includes('c ')) {
        ...
        if (dismissalText.includes('c & b') || dismissalText.includes('c and b')) {
          ...
          // Regular catches - using a more flexible regex
          const match = dismissalText.match(/c\s+(.*?)\s+(?:b|$)/);
          if (match) {
            let fielderName = match[1].trim();
            // Remove anything after 'b' if it exists
            fielderName = fielderName.split(' b')[0].trim();
            ...
            if (fielderName && !fielderName.includes('&') && !fielderName.includes('and')) {
              // Handle hyphenated names by preserving the hyphen
