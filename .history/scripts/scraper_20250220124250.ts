    ...
    if (inningsText.includes('Batter') || inningsText.includes('Batsman')) {
      ...
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        ...
        battingData.push({
            ...
            runs: parseInt(cols.eq(2).text()) || 0,
    ...
    if (inningsText.includes('Bowler')) {
      ...
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        ...
        bowlingData.push({
          ...
          runs: parseInt(cols.eq(3).text()) || 0,
...
function calculatePoints(playerData: PlayerData): {
  ...
  if (playerData.runs !== undefined) {
    ...
    if (playerData.fours) points.battingPoints += playerData.fours * 1;
    if (playerData.sixes) points.battingPoints += playerData.sixes * 2;
    ...
    const runMilestones = Math.floor(playerData.runs / 25);
    points.battingPoints += runMilestones * 10;
  }
  ...
  if (playerData.wickets !== undefined) {
    ...
    if (playerData.wickets >= 2) {
      points.bowlingPoints += (playerData.wickets - 1) * 10;
    }
    
    if (playerData.maidens) {
      points.bowlingPoints += playerData.maidens * 20;
    }

    if (playerData.overs && playerData.overs >= 1) {
      const economy = (playerData.runs || 0) / playerData.overs;
      if (economy < 4) points.bowlingPoints += 15;
      else if (economy < 5) points.bowlingPoints += 10;
      else if (economy < 6) points.bowlingPoints += 5;
      else if (economy > 9 && economy <= 10) points.bowlingPoints -= 5;
      else if (economy > 10 && economy <= 11) points.bowlingPoints -= 10;
      else if (economy > 11) points.bowlingPoints -= 15;
    }

    // Add dot ball points and log for debugging
    if (playerData.dotBalls) {
      console.log(`Adding ${playerData.dotBalls} dot ball points for ${playerData.fullName}`);
      points.bowlingPoints += playerData.dotBalls;
    } else {
      console.log(`No dot balls found for ${playerData.fullName}`);
    }

    if (playerData.wides) points.bowlingPoints -= playerData.wides / 2;
    if (playerData.noBalls) points.bowlingPoints -= playerData.noBalls * 2;
  }
...
function mergePlayerStats(battingData: PlayerData[], bowlingData: PlayerData[], fieldingData: PlayerData[], dnbData: PlayerData[]): PlayerData[] {
  ...
  const mergePlayer = (player: PlayerData) => {
    ...
    // Merge stats without logging
    if (player.runs !== undefined) {
      mergedPlayers[key].runs = (mergedPlayers[key].runs || 0) + player.runs;
      mergedPlayers[key].balls = (mergedPlayers[key].balls || 0) + (player.balls || 0);
      mergedPlayers[key].fours = (mergedPlayers[key].fours || 0) + (player.fours || 0);
      mergedPlayers[key].sixes = (mergedPlayers[key].sixes || 0) + (player.sixes || 0);
      mergedPlayers[key].strikeRate = player.strikeRate || 0;
    }

    if (player.overs !== undefined) {
      mergedPlayers[key].overs = (mergedPlayers[key].overs || 0) + player.overs;
      mergedPlayers[key].maidens = (mergedPlayers[key].maidens || 0) + (player.maidens || 0);
      mergedPlayers[key].wickets = (mergedPlayers[key].wickets || 0) + (player.wickets || 0);
      mergedPlayers[key].wides = (mergedPlayers[key].wides || 0) + (player.wides || 0);
      mergedPlayers[key].noBalls = (mergedPlayers[key].noBalls || 0) + (player.noBalls || 0);
      mergedPlayers[key].runs = (mergedPlayers[key].runs || 0) + (player.runs || 0);
...
async function main() {
  try {
    ...
    for (const match of matchesToProcess) {
      ...
      try {
        // First, save the match details to the matches table
        ...
        if (matchError) {
          console.error(`Error saving match ${match.id} details to database:`, matchError);
          continue;
        }
        */
        ...
        await exportPointsTable(playerPoints); // Save points to database
        displayPointsTable(playerPoints); // Just display the points
