...
const playerLookup: Record<string, { player: string; teamName: string; playerId: string }> = {};
...
async function loadPlayerData() {
    try {
        ...
        const playerLookup: Record<string, { playerId: string; teamName: string }> = {};

        // Populate lookup dictionary with both Player Name and Player ID
        playersData.forEach((player: any) => {
            ...
            // Also store by Player ID (if needed for reverse lookup)
            playerLookup[player["Player ID"]] = {
                playerId: player["Playee"],
                teamName: player["Team Name"]
            };
        });
...
async function updatePlayerData(playerPoints: PlayerPoints[]): Promise<PlayerPoints[]> {
  return playerPoints.map(player => {
      ...
      return {
          ...player,
          player_name: lookupData ? lookupData.player : "Unknown",
