...
async function parseScorecard(html: string, matchId: string): Promise<{
  ...
  $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').each((_, inningsElem) => {
    ...
    if (inningsText.includes('Batter') || inningsText.includes('Batsman')) {
      ...
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each(async (_, row) => {
        ...
        // Extract player ID and profile URL from hyperlink
        const href = playerLink.attr('href');
        const playerId = href ? href.split('/')[2] : null;
        const profileUrl = href ? `https://www.cricbuzz.com${href}` : null;

        // Get full name from profile URL
        ...
        if (profileUrl) {
          try {
            ...
            fullName = $profile('h1.cb-font-40').text().trim() || playerName;
          } catch (error) {
            console.error(`Error fetching full name for ${playerName}:`, error);
          }
        }
