...
async function processDNBPlayers($: cheerio.CheerioAPI, matchData: PlayerPoints[]) {
  const dnbPlayerIds = extractDNBPlayers($);

  dnbPlayerIds.forEach(playerId => {
      // Check if the player is already in matchData
      let existingPlayer = matchData.find(p => p.player_id === playerId);

      if (!existingPlayer) {
          // If missing, add them with zero points
          matchData.push({
              match_id: matchData[0]?.match_id || "Unknown",
              player_id: playerId,
              batting_points: 0,
              bowling_points: 0,
              fielding_points: 0,
