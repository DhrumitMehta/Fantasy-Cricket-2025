...
function calculatePoints(playerData: PlayerData): {
  ...
  if (playerData.runs !== undefined) {
    ...
    // 2. Boundary bonus (1 point per four, 2 points per six)
    if (playerData.fours) points.battingPoints += playerData.fours;
    ...
    // 4. Strike Rate bonus/penalty (only if 10+ balls faced)
    if (playerData.balls && playerData.balls >= 10) {
      ...
      else if (strikeRate < 125) points.battingPoints += 0;
      else if (strikeRate < 150) points.battingPoints += 5;
      else if (strikeRate < 200) points.battingPoints += 10;
      else points.battingPoints += 15;
    }
  }
  ...
  if (playerData.wickets !== undefined) {
    ...
    // 2. Wide ball penalty (-1 point per 2 wides)
    if (playerData.wides) {
      points.bowlingPoints -= Math.floor(playerData.wides / 2);
    }
    
    // 3. No ball penalty (-2 points per no ball)
    if (playerData.noBalls) {
      points.bowlingPoints -= playerData.noBalls * 2;
    }
    
    // 4. Wicket bonus (10 points for each wicket after first)
    if (playerData.wickets > 1) {
      points.bowlingPoints += (playerData.wickets - 1) * 10;
    }
    
    // 5. Economy rate bonus/penalty (minimum 1 over)
    if (playerData.overs && playerData.overs >= 1) {
      ...
      else if (economy < 9.01) points.bowlingPoints += 0;
      else if (economy < 10.01) points.bowlingPoints -= 5;
      else if (economy < 12.01) points.bowlingPoints -= 10;
      else points.bowlingPoints -= 20;
    }

    // Update dot ball points to 2 points per dot ball
    if (playerData.dotBalls) {
      console.log(`Adding ${playerData.dotBalls * 2} points for ${playerData.dotBalls} dot balls for ${playerData.fullName}`);
      points.bowlingPoints += playerData.dotBalls * 2;
    }
  }
