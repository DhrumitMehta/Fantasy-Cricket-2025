...
async function fetchWithRetry(url: string, maxRetries = 3, delay = 1000): Promise<string> {
  const headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
    'Accept-Language': 'en-US,en;q=0.5',
...
async function parseScorecard(html: string, matchId: string): Promise<{
  ...
  try {
    ...
    for (const inningsElem of $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').toArray()) {
      ...
      if (inningsText.includes('Batter') || inningsText.includes('Batsman')) {
        ...
        $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
          ...
          const playerName = cols.eq(0).find('a').text().trim();
          
          if (!playerName || playerName === 'Batter' || playerName === 'Batsman') return;
      ...
      if ($(inningsElem).find('.cb-scrd-sub-hdr').text().includes('Bowler')) {
        console.log('\nProcessing bowling section for innings', currentInnings);
        
        const bowlingTeam = teamNames[currentInnings % 2];
        const bowlerPromises: Promise<PlayerData>[] = [];
        ...
        $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
          ...
          // Extract player ID from the player profile link, not the highlights link
          const profileHref = playerLink.attr('href');
          const playerId = profileHref ? profileHref.split('/')[2] : null;
          
          console.log(`Player ${playerName} has ID: ${playerId}`);
          ...
          console.log(`Found bowler: ${playerName} (${bowlingTeam})`);
          console.log('Bowling stats:', bowlingStats);

          // Create a promise for fetching dot balls
          if (playerId) {
            const promise = getBowlerDotBalls(matchId, playerId, currentInnings.toString())
              .then(dotBalls => ({
                fullName: cleanPlayerName(playerName),
                shortName: cleanPlayerName(playerName),
                team: bowlingTeam,
            ...
            bowlerPromises.push(promise);
          }
        });

        // Wait for all dot ball data to be fetched
        const bowlers = await Promise.all(bowlerPromises);
        bowlingData.push(...bowlers);
      }

      // Process fielding data (catches, stumpings, run-outs)
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        const cols = $(row).find('.cb-col');
        const dismissalText = cols.eq(1).text().trim();
        
        // Skip if not a dismissal or if it's a batting row header
        if (!dismissalText || dismissalText === 'Batter' || dismissalText === 'Batsman' || 
            dismissalText.match(/^[\d.]+$/) || // Skip over/ball numbers
            dismissalText.includes(',')) // Skip player lists
          return;

        console.log(`Processing dismissal: ${dismissalText}`);

        // Process catches
        if (dismissalText.includes('c ')) {
          // Handle caught and bowled cases
          if (dismissalText.includes('c & b') || dismissalText.includes('c and b')) {
            const bowlerName = dismissalText.split(/c (?:&|and) b /)[1].trim();
            console.log(`Found caught & bowled: ${dismissalText} -> ${bowlerName}`);
            if (bowlerName) {
              const existingFielder = fieldingData.find(f => 
                cleanPlayerName(f.fullName) === cleanPlayerName(bowlerName) && 
                f.innings === currentInnings
              );
              if (existingFielder) {
                existingFielder.catches++;
              } else {
                fieldingData.push({
                  fullName: cleanPlayerName(bowlerName),
                  shortName: cleanPlayerName(bowlerName),
                  team: teamNames[currentInnings % 2],
                  matchId,
                  innings: currentInnings,
                  catches: 1,
                  stumpings: 0,
            ...
            console.log(`Found catch in innings ${currentInnings}: ${dismissalText} -> ${fielderName}`);
            
            if (fielderName && !fielderName.includes('&') && !fielderName.includes('and')) {
              // Preserve hyphenated names and handle regular names
              const cleanedName = fielderName.includes('-') ? 
                fielderName : 
                cleanPlayerName(fielderName);

              const existingFielder = fieldingData.find(f => 
                (f.fullName === cleanedName || cleanPlayerName(f.fullName) === cleanPlayerName(fielderName)) && 
                f.innings === currentInnings
              );
        ...
        if (dismissalText.includes('st ')) {
          const match = dismissalText.match(/st\s+([^b]+?)\s+b/);
          if (match) {
            const stumperName = match[1].trim();
            console.log(`Found stumping: ${dismissalText} -> ${stumperName}`);
            if (stumperName) {
              const existingFielder = fieldingData.find(f => cleanPlayerName(f.fullName) === cleanPlayerName(stumperName));
              if (existingFielder) {
                existingFielder.stumpings++;
              } else {
                fieldingData.push({
                  fullName: cleanPlayerName(stumperName),
                  shortName: cleanPlayerName(stumperName),
                  team: teamNames[currentInnings % 2],
                  matchId,
                  innings: currentInnings,
                  catches: 0,
        ...
        if (dismissalText.includes('run out')) {
          ...
          if (match) {
            const fielderNames = match[1].split('/').map(name => name.trim());
            console.log(`Found run out: ${dismissalText} -> ${fielderNames.join(', ')}`);
            fielderNames.forEach(fielderName => {
              if (fielderName) {
                const existingFielder = fieldingData.find(f => cleanPlayerName(f.fullName) === cleanPlayerName(fielderName));
                if (existingFielder) {
                  existingFielder.runOuts++;
                } else {
                  fieldingData.push({
                    fullName: cleanPlayerName(fielderName),
                    shortName: cleanPlayerName(fielderName),
                    team: teamNames[currentInnings % 2],
                    matchId,
                    innings: currentInnings,
                    catches: 0,
    ...
    // Log fielding data for verification
    console.log('\nFielding data collected:');
    fieldingData.forEach(fielder => {
      console.log(`${fielder.fullName} (${fielder.team}):`, {
    ...
    // Add debug logging for dismissals
    console.log('\nAll dismissals found:');
    $('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
      const dismissalText = $(row).find('.cb-col').eq(1).text().trim();
      if (dismissalText) {
    ...
    console.error(`Error parsing scorecard for match ${matchId}:`, error);
    return {
      battingData: [],
      bowlingData: [],
      fieldingData: [],
