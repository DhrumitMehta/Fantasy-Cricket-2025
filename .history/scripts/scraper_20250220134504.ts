...
async function parseScorecard(html: string, matchId: string): Promise<{
  ...
  // Process bowling data - look specifically for the bowling sections
  $('.cb-ltst-wgt-hdr').each((_, section) => {
    const sectionTitle = $(section).find('.cb-col.cb-col-100.head').text().trim();
    
    // Only process if this is a bowling section
    if (sectionTitle.includes('Bowling')) {
      console.log('\nProcessing bowling section:', sectionTitle);
      const currentBowlingTeam = sectionTitle.includes(teamNames[0]) ? teamNames[1] : teamNames[0];
      
      $(section).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        ...
        console.log(`Found bowler: ${playerName}`);
