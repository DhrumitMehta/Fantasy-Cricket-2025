...
async function parseScorecard(matchId: string): Promise<{
  ...
  try {
    ...
    // Get team names first - using the correct class
    ...
    // Create name mapping for consistent names across tables
      ...
      if (inningsText.includes('Batter') || inningsText.includes('Batsman')) {
        ...
        $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
          ...
          if (playerLink) {
            ...
            nameMapping[shortName.toLowerCase()] = fullName;
            nameMapping[fullName.toLowerCase()] = fullName;
          }
        });
      }
    });
