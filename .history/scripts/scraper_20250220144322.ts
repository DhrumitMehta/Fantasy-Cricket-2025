    ...
    $('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
      ...
      if (dismissalText.includes('c ')) {
        ...
        if (dismissalText.includes('c & b') || dismissalText.includes('c and b')) {
          ...
          if (bowlerName) {
            const existingFielder = fieldingData.find(f => cleanPlayerName(f.fullName) === cleanPlayerName(bowlerName));
            if (existingFielder) {
              existingFielder.catches = (existingFielder.catches || 0) + 1;
            } else {
              fieldingData.push({
      ...
      if (dismissalText.includes('st ')) {
        const stumperName = dismissalText.split('st ')[1].split(' b')[0].trim();
        if (stumperName) {
          const existingFielder = fieldingData.find(f => cleanPlayerName(f.fullName) === cleanPlayerName(stumperName));
          if (existingFielder) {
            existingFielder.stumpings = (existingFielder.stumpings || 0) + 1;
          } else {
            fieldingData.push({
      ...
      if (dismissalText.includes('run out')) {
        const runOutText = dismissalText.split('run out')[1].trim();
        if (runOutText && runOutText !== '()') {
          const fielderNames = runOutText.replace('(', '').replace(')', '').split('/');
          fielderNames.forEach(fielderName => {
            const trimmedName = fielderName.trim();
            if (trimmedName) {
              const existingFielder = fieldingData.find(f => cleanPlayerName(f.fullName) === cleanPlayerName(trimmedName));
              if (existingFielder) {
                existingFielder.runOuts = (existingFielder.runOuts || 0) + 1;
              } else {
                fieldingData.push({
                  fullName: cleanPlayerName(trimmedName),
                  shortName: cleanPlayerName(trimmedName),
                  team: teamNames[currentInnings % 2],
                  matchId,
                  innings: currentInnings,
                  catches: 0,
