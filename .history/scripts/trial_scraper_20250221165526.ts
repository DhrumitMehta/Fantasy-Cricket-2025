...
const nameVariationLookup: Record<string, string> = {}; // Maps name variations to full names

function addNameVariations(fullName: string) {
    const variations = generateNameVariations(fullName);
    variations.forEach(variation => {
        nameVariationLookup[variation] = fullName; // Store variations pointing to the full name
    });
}

// Convert Player IDs to full names and generate variations
function generateNameMappings(playerList: { playerId: string }[]) {
  playerList.forEach(player => {
      const fullName = playerLookup[player.playerId]?.playerName;
      if (fullName) {
          addNameVariations(fullName);
      }
  });
}

// Function to extract Did Not Bat (DNB) player IDs
function extractDNBPlayers($: cheerio.CheerioAPI): string[] {
  const dnbPlayers: string[] = [];

  // Locate "Did not Bat" section
  $('.cb-col-100.cb-scrd-itms').each((_, element) => {
      const sectionTitle = $(element).find('.cb-col-27').text().trim();
      
      if (sectionTitle === "Did not Bat") {
          // Extract player IDs from the <a> tags
          $(element).find('.cb-col-73 a').each((_, playerElement) => {
              const playerHref = $(playerElement).attr('href') || '';
              const playerIdMatch = playerHref.match(/\/profiles\/(\d+)\//);
              
              if (playerIdMatch) {
                  dnbPlayers.push(playerIdMatch[1]); // Store Player ID
              }
          });
      }
  });
...
async function processDNBPlayers($: cheerio.CheerioAPI, finalTable: any[]) {
  const dnbPlayerIds = extractDNBPlayers($);

  dnbPlayerIds.forEach(playerId => {
      // Check if the player already exists in the final table
      let existingPlayer = finalTable.find(p => p.playerId === playerId);

      if (!existingPlayer) {
          // If player is missing, add them with zero stats
          finalTable.push({
              playerId: playerId,
              playerName: playerLookup[playerId]?.playerName || "Unknown",
              teamName: playerLookup[playerId]?.teamName || "Unknown",
              batting_points: 0,
              bowling_points: 0,
              fielding_points: 0,
              potm_points: 0,
          ...
          console.log(`Added DNB player: ${playerLookup[playerId]?.playerName || "Unknown"} (Player ID: ${playerId})`);
      }
  });
}

// Function to extract DNB players
const extractDNBPlayers = async ($: cheerio.Root, currentInnings: number, battingTeam: string) => {
  const dnbPlayers: Array<[string, string, number, string]> = [];

  $('.cb-col.cb-col-100.cb-scrd-itms').each(async (_, element) => {
    if ($(element).text().includes('Did not Bat')) {
      const dnbLinks = $(element).find('a.cb-text-link');
      for (let i = 0; i < dnbLinks.length; i++) {
        const playerLink = $(dnbLinks[i]);
        const playerName = playerLink.text().trim();
        if (playerName) {
          const fullName = await fetchFullName(playerLink.attr('href') || '');
          dnbPlayers.push([fullName, playerName, currentInnings, battingTeam]);
        }
      }
    }
  });
...
async function parseScorecard(matchId: string): Promise<{
  ...
  try {
    ...
    for (const inningsElem of $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').toArray()) {
      ...
      // Generate mappings from all players in the match
      generateNameMappings(battingData);
      generateNameMappings(bowlingData);
      generateNameMappings(dnbData);

      console.log("Generated name variations for all match players.");
      ...
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        ...
        // Try to find the correct full name using name variations
        const fullName = nameVariationLookup[fielderName] || fielderName; // If no match, keep name

        // Get the Player ID using the full name
        let playerId = Object.keys(playerLookup).find(id => playerLookup[id]?.playerName === fullName) || fullName; // If no match, keep name

        console.log(`Mapped fielder name: "${fielderName}" → Full Name: "${fullName}" → Player ID: "${playerId}"`);

        // Find or create the fielder entry using Player ID
