...
$(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
  ...
  // Initialize fielding stats for this innings
  ...
  if (dismissalText.includes('c & b') || dismissalText.includes('c and b')) {
    ...
    // Handle regular catches using regex
    const catchMatch = dismissalText.match(/c\s+(.*?)\s+(?=b\s+)/);
    if (catchMatch) {
      const fielderName = catchMatch[1].trim();
      if (fielderName) {
        const existingFielder = fieldingData.find(f => 
          cleanPlayerName(f.fullName) === cleanPlayerName(fielderName) && 
          f.innings === currentInnings
        );
        if (existingFielder) {
          existingFielder.catches++;
  ...
  // Handle stumpings
  const stumpingMatch = dismissalText.match(/st\s+(\w+\s+\w+)/);
  if (stumpingMatch) {
    const stumperName = stumpingMatch[1].trim();
    if (stumperName) {
      const existingFielder = fieldingData.find(f => 
        cleanPlayerName(f.fullName) === cleanPlayerName(stumperName) && 
        f.innings === currentInnings
      );
      if (existingFielder) {
  ...
  // Handle run outs
  const runoutMatch = dismissalText.match(/run out\s*\(([\w\s/]+)\)/);
  if (runoutMatch) {
...
// Log all dismissals and fielding data for verification
...
fieldingData.forEach(fielder => {
  console.log(`${fielder.fullName} (${fielder.team}, Innings ${fielder.innings}):`, {
