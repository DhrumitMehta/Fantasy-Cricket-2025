  ...
  try {
    ...
    $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').each((_, inningsElem) => {
      ...
      if (inningsText.includes('Batter') || inningsText.includes('Batsman')) {
        ...
        $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
          ...
          if (playerLink) {
            ...
            const stats = cols.slice(1).map((_, i) => parseInt(cols.eq(i + 1).text()) || 0);
            playerData.battingRuns = stats[0];
            playerData.balls = stats[1];
            playerData.fours = stats[2];
            playerData.sixes = stats[3];
            playerData.strikeRate = stats[4];
      ...
      if (inningsText.includes('Bowler')) {
        const currentBowlingTeam = teamNames[currentInnings - 1];
        ...
        $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
          ...
          if (playerLink) {
            ...
            const stats = cols.slice(1).map((_, i) => parseInt(cols.eq(i + 1).text()) || 0);
            playerData.overs = stats[0];
            playerData.maidens = stats[1];
            playerData.runsConceded = stats[2];
            playerData.wickets = stats[3];
            playerData.noBalls = stats[4];
            playerData.wides = stats[5];
            playerData.economy = stats[6];
    ...
    // Process fielding data with corrected team assignment and name mapping
    $('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
      ...
      // Process catches, stumpings, and run outs with name mapping
      if (dismissalText.includes('c ')) {
        // ... existing catch processing code ...
        const fielderName = // ... get fielder name ...
        const mappedName = nameMapping[fielderName.toLowerCase()] || fielderName;
        // Use mappedName when creating fielding data
      }
      // ... rest of fielding processing ...
    });
    ...
    if (potmDiv.length) {
      ...
      const mappedPotmName = nameMapping[potmName.toLowerCase()] || potmName;
      console.log(`Player of the Match: ${mappedPotmName}`);
      // Add POTM points to the player's data
    }
