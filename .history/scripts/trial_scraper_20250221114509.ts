...
const TEST_MATCH_IDS = [
  '47351', // Add a few known match IDs here
  '47352',
  '47353'
];
...
async function testScraper() {
  console.log('Testing scraper with sample match IDs...');
  ...
  for (const matchId of TEST_MATCH_IDS) {
    ...
    if (stats) {
      console.log('✓ Batting stats count:', stats.battingStats.length);
      console.log('✓ Bowling stats count:', stats.bowlingStats.length);
      console.log('✓ Fielding stats count:', stats.fieldingStats.length);
      
      // Log sample data
      if (stats.battingStats.length > 0) {
        console.log('Sample batting stat:', stats.battingStats[0]);
      }
      if (stats.bowlingStats.length > 0) {
        console.log('Sample bowling stat:', stats.bowlingStats[0]);
      }
    } else {
      console.log('✗ Failed to fetch match stats');
    }
    
    // Additional delay between test matches
    await delay(2000);
  }
}

// Export test function
export { getMatchStats, testScraper };