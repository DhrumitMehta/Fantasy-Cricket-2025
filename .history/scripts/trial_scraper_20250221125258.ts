...
const playerLookup: Record<string, { player: string; teamName: string }> = {};

// Load players_with_prices.json and populate the lookup dictionary
async function loadPlayerData() {
    try {
        const playersFilePath = 'players_with_prices.json'; // Adjust path if needed
  ...
  try {
    ...
    for (const inningsElem of $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').toArray()) {
      ...
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        ...
        // TODO: In the future, we'll need to map this fielder name to their player ID
        // For now, we'll store the name as the ID
        const playerId = fielderName;
...
// Function to update player data with names and teams
async function updatePlayerData(playerPoints: PlayerPoints[]): Promise<PlayerPoints[]> {
  return playerPoints.map(player => ({
      ...player,
      player_name: playerLookup[player.player_id]?.player || "Unknown",
      team_name: playerLookup[player.player_id]?.teamName || "Unknown"
  }));
}
  ...
  try {
    ...
    for (const match of matchesToProcess) {
      ...
      try {
        ...
        processMatchData(exampleMatchData);
