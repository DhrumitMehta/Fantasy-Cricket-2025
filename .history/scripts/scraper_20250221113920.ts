          ...
          if (cols.length >= 7) {
            ...
            if (playerName && !playerName.includes('Did not Bat')) {
              ...
              if (player) {
                ...
                // Create new player with all stats initialized to 0
                player = {
                  fullName,
        ...
        $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
          ...
          if (cols.length >= 8) {
            ...
            if (playerName) {
              ...
              // Try to find existing player
              let player = bowlingData.find(p => 
                p.fullName.toLowerCase() === fullName.toLowerCase() || 
                p.shortName.toLowerCase() === playerName.toLowerCase()
              );

              if (player) {
                // Update only bowling stats
                player.overs = overs;
                ...
                // Update dot balls asynchronously
                getBowlerDotBalls(matchId, playerId, currentInnings)
                  .then(dotBalls => {
                ...
                player = {
                  ...
                  economy,
                  runsConceded: runs,
                  dotBalls: 0,  // Will be updated asynchronously
