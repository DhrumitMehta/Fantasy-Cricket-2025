  ...
  try {
    ...
    for (const inningsElem of $('.cb-col.cb-col-100.cb-ltst-wgt-hdr').toArray()) {
      ...
      $(inningsElem).find('.cb-col.cb-col-100.cb-scrd-itms').each((_, row) => {
        ...
        // Try to find a matching player ID using name variations
        let playerId = nameVariations
        .map(name => playerLookup[name]?.playerId) // Lookup Player ID using name variations
        .find(id => id !== undefined) || fielderName; // If no match, keep original name
